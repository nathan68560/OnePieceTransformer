...
class Berryizer():
    def __init__(self):
        ...
        """
        Regex pattern to separate the input text into chunks matching the pattern. This prevent the creation of 'cross-words' tokens.
        1. ' ?\p{L}'            -> match an optional space followed by any combination of letter code points (ie: words)
        2. ' ?\p{N}'            -> match an optional space followed by any combination of numeric code points (ie: numbers)
        3. ' ?[^\s\p{L}\p{N}]+' -> match an optional space followed by any combination of characters that are neither a letter, a numeric, or a whitespace (ie: ponctuations)
        4. '\s+(?!\S)'          -> match any combination of whitespaces THAT AREN'T followed by a non-whitespace (ie: multiple whitespaces until reaching the match condition for 1,2 or 3)
        5. '\s+'                -> match any combination of whitespaces (ie: trailing whitespaces)
        """
        self.compiled_regpat = re.compile(self.regpat)
        self.prefixer = r"""(?<=^) ?(?=[\p{L}\p{N}])"""
        """
        Regex pattern to select every new line's first word.
        Used to add a temporary space at the start of new lines to prevent the generation of almost-duplicate tokens.
        """
        self.compiled_prefixer = re.compile(self.prefixer, flags=re.MULTILINE)
    ...
    def load(self, model_file="Berryizer.model"):
        ...
        try:
            with open(model_file, 'r', encoding="utf-8") as f:
                # Read the pattern
                self.regpat = f.readline().strip().replace('regpat:', '')
                self.compiled_regpat = re.compile(self.regpat)
                # Read the merges
                for line in f:
    ...
    def train(self, text, vocab_size):
        ...
        # Add a temporary space before every new line's first word
        text = re.sub(self.compiled_prefixer, ' ', text)
    ...
    def decode(self, tokens):
        ...
        text = bytestring.decode("utf-8", errors="replace")
        return re.sub(self.compiled_prefixer, '', text)